const apibath = 'http://192.168.101.20:8801/' //API根路径
// const apibath = 'http://ecs.treetao.com/';
//简化uni.request
...
/*
var that = this;
	this.$get('mall/sku/commonpage?isShumbInfo=true',{current:1,cataid:this.cataid},'get',true).then((res) => {
		//console.log(res)
		if (res.data) {
			for (var k = 0; k < res.data.items.length; k++) {
				var item = {
					id: res.data.items[k].id,
					uri: res.data.items[k].thumbnail,
					title: res.data.items[k].name,
					specialprice: '￥' + res.data.items[k].priceSale,
					originprice: res.data.items[k].priceDel?'￥'+ res.data.items[k].priceDel:''
				}
				that.geusts.datas.push(item)
			}
		}
	});
*/